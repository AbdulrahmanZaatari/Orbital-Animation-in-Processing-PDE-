...
void updateOrbiters(double timeDelta) {
  ...
  while (!queue.isEmpty()) {
    Orbiter current = queue.remove();
    current.updatePosition(timeDelta); // This method might need to be implemented or updated in the Orbiter class
    for (Orbiter child : current.getChildren()) {
      queue.add(child);
    }
  }
}
...
void drawOrbiters() {
  ...
  while (!queue.isEmpty()) {
    Orbiter current = queue.remove();
    drawOrbiter(current); // Draws the orbiter
